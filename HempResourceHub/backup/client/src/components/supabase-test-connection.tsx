...
export default function SupabaseTestConnection() {
  ...
  useEffect(() => {
    ...
    if (url) {
      setSupabaseUrl(typeof url === 'string' ? `${url.substring(0, 15)}...` : 'Invalid format');
    } else {
  ...
  async function handleTestConnection() {
    ...
    try {
      ...
      if (!url || !key) {
        ...
        setMessage('Missing Supabase credentials. Please check environment variables.');
        return;
      }

      console.log('Creating Supabase client with URL:', typeof url, url ? url.substring(0, 10) + '...' : 'none');
      
      // Create client directly in this component
      const supabase = createClient(url, key);
      
      // Test the connection
      const { data, error } = await supabase.from('plant_types').select('count').limit(1);
      ...
      setErrorDetails(err instanceof Error ? err.stack || 'No stack trace' : 'Unknown error type');
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setConnectionStatus('error');
          setMessage(`Failed to reach ${testUrl}: ${response.status} ${response.statusText}`);
        }
      } catch (fetchErr) {
        setConnectionStatus('error');
        setMessage(`Network error reaching ${testUrl}`);
        setErrorDetails(fetchErr instanceof Error ? fetchErr.message : String(fetchErr));
      }
    } catch (err) {
      setConnectionStatus('error');
    ...
    <Card className="w-full max-w-lg mx-auto mt-6">
      ...
      <CardContent>
        <div className="space-y-4">
          <Alert variant={supabaseUrl && supabaseUrl !== 'Not found' ? "default" : "destructive"}>
          ...
          <Alert variant={
            ...
            <AlertDescription className="mt-2">
              {message}
              {errorDetails && (
                <div className="mt-2 p-2 bg-gray-100 rounded text-xs font-mono whitespace-pre-wrap max-h-48 overflow-y-auto">
                  {errorDetails}
                </div>
              )}
            </AlertDescription>
          ...
          <div className="text-sm text-gray-500 mt-4">
            <p>If you're seeing connection errors, here are some possible reasons:</p>
            <ul className="list-disc pl-5 mt-2 space-y-1">
              <li>Supabase URL or key might be incorrect</li>
              <li>CORS policies might be blocking access</li>
              <li>Network connectivity issues</li>
              <li>The tables may not exist in your Supabase project</li>
            </ul>
          </div>
        </div>
      </CardContent>
